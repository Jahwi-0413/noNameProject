...
public class UserMainController
{
    ...
    public void clickTour() throws IOException
    {
        settingWindow("tour");
    }
    public void clickRestrant() throws IOException
    {
        settingWindow("restaurant");
    }
    public void clickLodg() throws IOException
    {
    ...
    public void settingWindow(String clickedButton) throws IOException      //버튼 눌렀을 때 화면 설정하는거 함수로 따로 뺌
    {
        ...
        switch(clickedButton)       //버튼 클릭의 경우에 따라 달라지는 설정을 뺌
        {
            case "tour":
                ...
                second= FXMLLoader.load(getClass().getResource("usermain_tour.fxml"));
                break;
            case "restaurant":
                tourClicked=0; restaurantClicked=1; lodgmentClicked=0;
                second=FXMLLoader.load(getClass().getResource("usermain_restaurant.fxml"));
                break;
            case "lodgment":
                tourClicked=0; restaurantClicked=0; lodgmentClicked=1;
                second=FXMLLoader.load(getClass().getResource("usermain_lodgment.fxml"));
                break;
            default:
                second=null;
        }
    ...
    {
        String keyWord = textField.getText(); //검색어를 들고온다
        if(!keyWord.equals("")) //키워드를 입력한 경우
        {
            if(tourClicked==1)
            {
                AdminServerConnector.getPacketManager().searchRequest(PW_USER , TAG_TRAVEL,keyWord);
            }
            else if(restaurantClicked==1)
            {
                AdminServerConnector.getPacketManager().searchRequest(PW_USER ,TAG_FOOD , keyWord);
            }
            else if(lodgmentClicked==1)
            {
                AdminServerConnector.getPacketManager().searchRequest(PW_USER ,TAG_HOTEL,keyWord);
            }
        }
        else
        {
            if(tourClicked==1)
            {
                AdminServerConnector.getPacketManager().searchRequest(PW_USER, TAG_TRAVEL);
            }
            else if(restaurantClicked==1)
            {
                AdminServerConnector.getPacketManager().searchRequest(PW_USER , TAG_FOOD);
            }
            else if(lodgmentClicked==1)
            {
                AdminServerConnector.getPacketManager().searchRequest(PW_USER ,TAG_HOTEL);
            }
        }
